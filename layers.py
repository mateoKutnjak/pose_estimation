...
def residual_module(input, channels_out, layer_name=''):
    ...
    x = Conv2D(filters=channels_out // 2, kernel_size=(1, 1), padding='same', activation='relu', name=layer_name + '_conv_1x1_(1)')(input)
    x = BatchNormalization()(x)
    x = Conv2D(filters=channels_out // 2, kernel_size=(3, 3), padding='same', activation='relu', name=layer_name + '_conv_3x3_(2)')(x)
    x = BatchNormalization()(x)
    x = Conv2D(filters=channels_out, kernel_size=(1, 1), padding='same', activation='relu', name=layer_name + '_conv_1x1_(3)')(x)
    x = BatchNormalization()(x)
...
def create_front_module(input, channels_out):
    x = Conv2D(filters=64, kernel_size=(7, 7), stride=(2, 2), padding='same', activation='relu', name='front_module_conv2d_7x7')(input)
    x = BatchNormalization()(x)
...
def create_single_hourglass_module(input, channels, classes, layer_num):
    ...
    x = residual_module(x, channels, layer_name=layer_num + '_middle_layer_1')(x)
    x = residual_module(x, channels, layer_name=layer_num + '_middle_layer_2')(x)
    x = residual_module(x, channels, layer_name=layer_num + '_middle_layer_3')(x)
...
def intermediate_supervision(prev_output, curr_output, channels, classes, layer_num):
    x = Conv2D(channels, kernel_size=(1, 1), padding='same', activation='relu', name=layer_num + '_head_conv_1x1_(1)')(curr_output)
    x = BatchNormalization()(x)
    h1 = Conv2D(channels, kernel_size=(1, 1), padding='same', activation='linear', name=layer_num + '_head_conv_1x1_(2)')(x)

    heatmaps = Conv2D(classes, kernel_size=(1, 1), padding='same', activation='linear', name=layer_num + '_head_conv_1x1_(3)->heatmap')(h1)
    h2 = Conv2D(channels, kernel_size=(1, 1), padding='same', activation='linear', name=layer_num + '_head_conv_1x1_(3)->heatmap')(heatmaps)
